...
export type GuidedLiveAccess={token:string;model:string;websocketUrl:string;voice:string;remainingSeconds:number};
...
export class GeminiGuidedPhraseLive {
  ...
  async connect(access:GuidedLiveAccess){
    ...
    await new Promise<void>((resolve,reject)=>{
      ...
      socket.onopen=()=>{clearTimeout(timeout);socket.send(JSON.stringify({setup:{model:`models/${access.model}`,responseModalities:['AUDIO']}}));resolve();};
      socket.onerror=()=>{clearTimeout(timeout);reject(new Error('Gemini Live connection failed.'));};
      socket.onmessage=(event)=>this.handleMessage(String(event.data));
      socket.onclose=()=>this.callbacks.onSpeaking(false);
    });
  }
  ...
  close(){this.socket?.close();this.socket=null;void this.audioContext.suspend();}

  private send(value:unknown){if(this.socket?.readyState===WebSocket.OPEN)this.socket.send(JSON.stringify(value));}
  private handleMessage(raw:string){
    try{
      const message=JSON.parse(raw);
      if(message.setupComplete){this.callbacks.onConnected();this.begin();return;}
    ...
    }catch(error){this.callbacks.onError(error instanceof Error?error.message:'Gemini returned an unreadable message.');}
  }
