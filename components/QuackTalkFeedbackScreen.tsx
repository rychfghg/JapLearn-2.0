...
type Session = {
  ...
  turnAssessments?: Array<{ recognizedText?: string; pronunciation?: number; accuracy?: number; fluency?: number; completeness?: number }>;
};
        ...
        {!loading && !error ? <>
          ...
          {!visible.length ? <Empty icon={mode === 'guided' ? 'mic-outline' : 'chatbubbles-outline'} title={mode === 'guided' ? 'No assessed phrases yet' : 'No conversations saved yet'} copy={mode === 'guided' ? 'Complete a Guided Phrase practice to see pronunciation scores and Sumi’s coaching here.' : 'Completed Talk with Sumi sessions will appear here as conversation activity.'} /> : visible.map((session, index) =>
            <View key={session.id || `${session.practicedAt}-${index}`} style={styles.sessionCard}>
              ...
              </> : <><View style={styles.activityRow}><View style={styles.activityItem}><Text style={styles.activityValue}>{session.conversationTurns || '—'}</Text><Text style={styles.activityLabel}>Conversation turns</Text></View><View style={styles.activityItem}><Text style={styles.activityValue}>{rounded(session.score) ?? '—'}</Text><Text style={styles.activityLabel}>Overall score</Text></View></View>{session.evaluated?<View style={styles.metricStrip}>{metric('Pronunciation',session.pronunciationScore)}{metric('Accuracy',session.accuracyScore)}{metric('Fluency',session.fluencyScore)}{metric('Conversation',session.contextualAccuracy)}</View>:null}{session.feedbackSummary?<View style={styles.coachNote}><Text style={styles.detailLabel}>SUMI'S SESSION NOTE</Text><Text style={styles.detailText}>{session.feedbackSummary}</Text></View>:null}{session.turnAssessments?.length?<View style={styles.detailBlock}><Text style={styles.detailLabel}>INDIVIDUAL PRONUNCIATION</Text>{session.turnAssessments.map((item,turn)=><Text key={turn} style={styles.detailText}>Response {turn+1}: {rounded(item.pronunciation) ?? '—'}/100 · accuracy {rounded(item.accuracy) ?? '—'} · fluency {rounded(item.fluency) ?? '—'}</Text>)}</View>:null}{session.areasForImprovement?.length?<View style={styles.focusBlock}><Text style={styles.focusLabel}>FOCUS NEXT</Text><Text style={styles.detailText}>{session.areasForImprovement.join(' · ')}</Text></View>:null}</>}
            </View>)}
