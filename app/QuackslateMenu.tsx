...
import { Image, Modal, Platform, Pressable, SafeAreaView, StyleSheet, Text, TextInput, View } from 'react-native';
    ...
    <SafeAreaView style={s.screen}>
      ...
      <View style={s.sessionSheet}>
        ...
        <Pressable style={({ pressed }) => [s.sessionOption, pressed && s.pressed]} onPress={() => setJoinOpen(true)}>
          <View style={s.optionNumber}><View style={s.optionIconHalo}><Ionicons name="people-outline" size={25} color="#8423D9" /></View></View>
        ...
        <Pressable style={({ pressed }) => [s.sessionOption, s.soloOption, pressed && s.pressed]} onPress={() => router.push({ pathname: '/Quackslate', params: { mode: 'system' } })}>
          <View style={s.optionNumberSolo}><View style={s.optionIconHaloSolo}><Ionicons name="extension-puzzle-outline" size={25} color="#FFFFFF" /></View></View>
      ...
      <Modal visible={guideOpen} transparent animationType="fade" onRequestClose={() => setGuideOpen(false)}>
        <View style={s.modalShade}>
          <View style={s.guideModal}>
            ...
            <View style={s.guideGlow} />
            <View style={s.guideSeal}><Ionicons name="extension-puzzle-outline" size={31} color="#FFFFFF" /></View>
            <View style={s.guideBadge}><Ionicons name="sparkles" size={13} color="#6FAA3F" /><Text style={s.modalKicker}>QUACKSLATE QUICK GUIDE</Text></View>
            <Text style={s.guideTitle}>Build the sentence, one tile at a time</Text>
            <Text style={s.guideText}>Follow the meaning, arrange the Japanese tiles, and check your sentence before the clock reaches zero.</Text>
            <View style={s.guideSteps}>
              <View style={s.guideStep}><View style={[s.stepNumber,s.stepPurple]}><Ionicons name="albums-outline" size={17} color="#8423D9" /></View><View style={s.stepCopy}><Text style={s.stepTitle}>Choose how to play</Text><Text style={s.stepText}>Enter a teacher code or begin a solo practice round.</Text></View></View>
              <View style={s.guideStep}><View style={[s.stepNumber,s.stepGreen]}><Ionicons name="hand-left-outline" size={17} color="#69A63D" /></View><View style={s.stepCopy}><Text style={s.stepTitle}>Arrange the Japanese tiles</Text><Text style={s.stepText}>Tap each word in the order that builds the complete sentence.</Text></View></View>
              <View style={s.guideStep}><View style={[s.stepNumber,s.stepGold]}><Ionicons name="timer-outline" size={17} color="#D68A1C" /></View><View style={s.stepCopy}><Text style={s.stepTitle}>Use your 15 seconds wisely</Text><Text style={s.stepText}>Check the sentence before time ends, then learn from the explanation.</Text></View></View>
            </View>
...
const s = StyleSheet.create({
  ...
  modalShade:{flex:1,backgroundColor:'rgba(31,11,39,.68)',alignItems:'center',justifyContent:'center',padding:22},joinModal:{width:'100%',maxWidth:370,borderRadius:29,backgroundColor:'#FFFFFF',padding:25,alignItems:'center'},guideModal:{width:'100%',maxWidth:390,borderRadius:31,backgroundColor:'#FFFFFF',padding:25,alignItems:'center',overflow:'hidden',borderWidth:1,borderColor:'rgba(255,255,255,.5)',shadowColor:'#24102D',shadowOpacity:.28,shadowRadius:24,elevation:14},guideGlow:{position:'absolute',top:-90,width:250,height:180,borderRadius:120,backgroundColor:'#F0DDFB'},closeButton:{position:'absolute',right:16,top:16,width:38,height:38,borderRadius:13,backgroundColor:'#F5F1F7',alignItems:'center',justifyContent:'center',zIndex:2},guideSeal:{width:70,height:70,borderRadius:24,backgroundColor:'#8423D9',alignItems:'center',justifyContent:'center',shadowColor:'#8423D9',shadowOpacity:.24,shadowRadius:12,elevation:5},guideBadge:{flexDirection:'row',alignItems:'center',gap:6,marginTop:16,paddingHorizontal:10,paddingVertical:6,borderRadius:99,backgroundColor:'#F2F8EC'},guideTitle:{fontFamily:'Jua',fontSize:23,lineHeight:28,color:'#3B2146',textAlign:'center',marginTop:9},guideText:{fontSize:11,lineHeight:17,color:'#817486',textAlign:'center',marginTop:6},guideSteps:{width:'100%',borderRadius:20,padding:14,gap:10,marginTop:17,backgroundColor:'#FAF7FC',borderWidth:1,borderColor:'#EEE5F1'},guideStep:{flexDirection:'row',alignItems:'center',paddingBottom:9,borderBottomWidth:1,borderBottomColor:'#EEE6F1'},stepNumber:{width:35,height:35,borderRadius:12,alignItems:'center',justifyContent:'center',marginRight:11},stepPurple:{backgroundColor:'#EDE0F8'},stepGreen:{backgroundColor:'#EDF7E7'},stepGold:{backgroundColor:'#FFF1DE'},stepCopy:{flex:1},stepNumberText:{fontFamily:'Jua',fontSize:13,color:'#8423D9'},stepTitle:{fontFamily:'Jua',fontSize:13,color:'#472751'},stepText:{fontSize:9,lineHeight:13,color:'#8A7D8F',marginTop:2},guideButton:{width:'100%',height:52,borderRadius:16,backgroundColor:'#8423D9',flexDirection:'row',alignItems:'center',justifyContent:'center',gap:8,marginTop:18,shadowColor:'#8423D9',shadowOpacity:.2,shadowRadius:10,elevation:4},guideButtonText:{fontFamily:'Jua',fontSize:12,color:'#FFFFFF'},modalIcon:{width:64,height:64,borderRadius:22,backgroundColor:'#F0E3FA',alignItems:'center',justifyContent:'center'},modalKicker:{fontSize:8,fontWeight:'900',letterSpacing:1.2,color:'#69AA3E'},modalTitle:{fontFamily:'Jua',fontSize:24,color:'#3B2146',marginTop:6},modalText:{fontSize:11,color:'#817486',textAlign:'center',marginTop:4},codeInput:{width:'100%',height:63,borderRadius:17,backgroundColor:'#F7F3F9',borderWidth:1,borderColor:'#DDD1E3',color:'#572666',fontSize:24,fontWeight:'900',letterSpacing:9,textAlign:'center',marginTop:20},errorText:{color:'#C75065',fontSize:10,lineHeight:15,textAlign:'center',marginTop:9},joinButton:{width:'100%',height:52,borderRadius:16,backgroundColor:'#8423D9',flexDirection:'row',alignItems:'center',justifyContent:'center',gap:8,marginTop:15},joinButtonText:{fontFamily:'Jua',fontSize:12,color:'#FFFFFF'},
  sessionSheet:{flex:1,marginTop:-18,borderTopLeftRadius:32,borderTopRightRadius:32,backgroundColor:'#FBF9FC',paddingHorizontal:22,paddingTop:12,elevation:12},
  sessionOption:{minHeight:105,borderRadius:23,backgroundColor:'#FFFFFF',borderWidth:1,borderColor:'#E4D9E8',padding:16,flexDirection:'row',alignItems:'center',shadowColor:'#3B2146',shadowOpacity:.1,shadowRadius:14,shadowOffset:{width:0,height:7},elevation:4},
  soloOption:{backgroundColor:'#FFFCFF',borderColor:'#D9C5E6'},
  optionNumber:{width:56,height:56,borderRadius:19,backgroundColor:'#F1E5FA',alignItems:'center',justifyContent:'center',borderWidth:1,borderColor:'#E2CFF0'},
  optionIconHalo:{width:42,height:42,borderRadius:14,backgroundColor:'#FFFFFF',alignItems:'center',justifyContent:'center'},
  optionNumberSolo:{width:56,height:56,borderRadius:19,backgroundColor:'#8423D9',alignItems:'center',justifyContent:'center',shadowColor:'#8423D9',shadowOpacity:.22,shadowRadius:9,elevation:4},
  optionIconHaloSolo:{width:42,height:42,borderRadius:14,backgroundColor:'rgba(255,255,255,.14)',alignItems:'center',justifyContent:'center'},
});
